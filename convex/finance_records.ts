  ...
  handler: async (ctx, args) => {
    ...
    if (args.record_type) {
      records = await ctx.db
        ...
        .withIndex('by_record_type', (q) =>
          q.eq('record_type', args.record_type as 'income' | 'expense')
        )
        .collect();
    } else if (args.status) {
      records = await ctx.db
...
export const getDashboardMetrics = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Get pending counts
    const pendingIncome = filteredRecords.filter(
      (r) => r.record_type === 'income' && r.status === 'pending'
    ).length;

    const pendingExpenses = filteredRecords.filter(
      (r) => r.record_type === 'expense' && r.status === 'pending'
    ).length;
...
export const getVaultBalance = query({
  ...
  handler: async (ctx) => {
    const records = await ctx.db
      .query('finance_records')
      .filter((q) =>
        q.and(q.eq(q.field('payment_method'), 'cash'), q.eq(q.field('status'), 'approved'))
      )
      .collect();
